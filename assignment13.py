	...
	if session['logged_in'] != True:
		...
		if request.method == 'GET':
			...
			if request.form['first_name'] == "":
				...
				try:
					g.db.execute('insert into STUDENTS (FIRSTNAME, LASTNAME) values (?,?)',(request.form['first_name'], request.form['last_name']))
					g.db.commit()
					return redirect('/dashboard')
				except:
	...
	if session['logged_in'] != True:
		...
		if request.method == 'GET':
			...
			if request.form['subject'] == "":
				...
				try:
					g.db.execute('insert into QUIZZES (SUBJECT, QUESTION_COUNT, QUIZ_DATE) values (?,?,?)',(request.form['subject'],request.form['question_count'],request.form['date']))
					g.db.commit()
					return redirect('/dashboard')
				except:
					flash("Error Updating Record")
